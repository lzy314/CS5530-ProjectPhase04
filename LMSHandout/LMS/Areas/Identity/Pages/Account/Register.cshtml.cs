...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        /// <summary>
        /// Scans the Students, Professors, and Administrators tables to generate the next unique uID.
        /// </summary>
        /// <returns>A string like "u0000001" etc. </returns>
        private string GenerateNewUid()
        {
            var allStudentIds = db.Students.Select(s => s.UId);
            var allProfessorIds = db.Professors.Select(p => p.UId);
            var allAdminIds = db.Administrators.Select(a => a.UId);
            var allIds = allStudentIds.Concat(allProfessorIds).Concat(allAdminIds);

            int maxId = 0;
            foreach (var id in allIds)
            {
                if (!string.IsNullOrEmpty(id) && id.Length == 8 && id[0] == 'u')
                {
